...
-- Define the expectedMinutesInOven constant to check how many minutes the
...
-- Define the preparationTimeInMinutes function that takes the number of layers
...
-- Define the elapsedTimeInMinutes function that takes two parameters: the first
...
-- Just set it to a straight integer. Nothing fancy.
expectedMinutesInOven :: Int
expectedMinutesInOven = 40

-- Basically, dooble the input. Two minutes per layer. Layers * 2.
preparationTimeInMinutes :: Int -> Int
preparationTimeInMinutes numberOfLayers = numberOfLayers * 2

-- Takes two integers. Calculates the prep time (using the function above, which
-- is simply doubling the number of layers passed in). Then adds the second
-- integer that was passed in (the elapsed time in oven).
elapsedTimeInMinutes :: Int -> Int -> Int
elapsedTimeInMinutes numberOfLayers elapsedTimeInOvenInMinutes =
  preparationTimeInMinutes numberOfLayers + elapsedTimeInOvenInMinutes
